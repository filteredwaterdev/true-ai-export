/**
 * true-ai-export — Claude.ai Conversation Exporter (Markdown, Clean)
 ...
 * conversation only — the back-and-forth between you and Claude
 * as plain readable text. Tool use, tool results, and internal
 ...
 * explicitly, use export-markdown.js instead.
 *
 ...
 *       are omitted — these are internal technical operations
 *       not part of the readable conversation
 ...
 *     this version — use export-markdown.js for full output
...
async function trueAiExportMarkdownClean() {
  ...
  // ─── Clean Markdown Builder ──────────────────────────────────────────────

  function buildMarkdownClean(messages, conversationName) {
    let humanCount = 0;
    let claudeCount = 0;
    let skippedCount = 0;
    let warningCount = 0;
    ...
    markdown +=
      ...
      `filteredwaterdev/true-ai-export) — Markdown clean v1.2.0*\n\n`;
    markdown += `---\n\n`;
      ...
      for (const block of contentBlocks) {
        ...
        if (block.type === 'text') {
          ...
          markdown +=
            ...
            `could not be captured as plain text.\n`;
          if (name) {
    ...
    markdown += `| Attachments / images flagged | ${warningCount} |\n`;
    markdown +=
    ...
    if (warningCount > 0) {
      ...
      markdown +=
        ...
        `retrieve any content that could not be exported.\n\n`;
    }
    ...
    if (omittedCount > 0) {
      ...
      markdown +=
        `> Use \`export-markdown.js\` for the full version ` +
        `with all blocks flagged.\n\n`;
    }

    if (warningCount === 0 && omittedCount === 0) {
      markdown +=
        `> ✅ Clean export complete. All messages exported ` +
        `as plain text with no omissions.\n\n`;
    }
  ...
  try {
    ...
    const {
      ...
      warningCount,
      omittedCount
    } = buildMarkdownClean(messages, conversationName);
    ...
    const filename =
      sanitizeFilename(conversationName) + '-clean.md';

    // Always store on window before any delivery attempt —
    // available as manual fallback in the console regardless of browser:
    //   copy(window._trueAiLastExport.content)
    ...
    if (isSafari) {
      ...
      btn.onclick = async () => {
        try {
          ...
          btn.textContent = `✅ Copied — paste into TextEdit, save as "${filename}"`;
          btn.style.color = '#1B5E20';
          btn.disabled = true;
          console.log(
            `[true-ai-export] Clipboard copy successful.`
          );
          console.log(
          ...
          // Real click still blocked — fall back to DevTools copy()
          btn.textContent =
      ...
      // Stay visible for 60 seconds so the user has time to notice
