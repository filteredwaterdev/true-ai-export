/**
 * true-ai-export — Claude.ai Conversation Exporter (PDF, Sans-Serif)
 ...
 *
 * This is the SANS-SERIF version. It exports the complete
 * conversation as a print-optimised document using a sans-serif
 * typeface — suited to general professional use. Every non-text
 * content block is flagged inline with a clearly visible note.
 * Nothing is silently dropped. Nothing is invisibly absent.
 *
 * For the clean version that omits tool use and internal
 * reasoning blocks, use export-pdf-clean-sans.js.
 * For the serif version use export-pdf-serif.js.
 *
...
async function trueAiExportPdfSans() {
  ...
  // ─── PDF Builder (Sans-Serif) ─────────────────────────────────────────────

  function buildPdfSans(messages, conversationName) {
    let humanCount = 0;
    let claudeCount = 0;
    let skippedCount = 0;
    let warningCount = 0;
...
<body>
  ...
  try {
    ...
    const {
      ...
      skippedCount,
      warningCount
    } = buildPdfSans(messages, conversationName);
    ...
    console.log(
      `[true-ai-export] PDF sans-serif v1.2.0 — ` +
