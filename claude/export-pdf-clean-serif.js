/**
 * true-ai-export — Claude.ai Conversation Exporter (PDF, Clean, Serif)
 ...
 *
 * This is the CLEAN SERIF version. It exports the human-readable
 * conversation only as a print-optimised document using a serif
 * typeface — suited to formal, legal, and archival use. Tool use,
 * tool results, and internal reasoning blocks are omitted.
 * Attachments and images are flagged since they directly affect
 * what was visible in the conversation.
 *
 * For the sans-serif version use export-pdf-clean-sans.js.
 * For the full version that flags every non-text content block
 * explicitly, use export-pdf-serif.js.
 *
 ...
 *     this version — use export-pdf-serif.js for full output
...
async function trueAiExportPdfCleanSerif() {
  ...
  // ─── Clean PDF Builder (Serif) ───────────────────────────────────────────

  function buildPdfCleanSerif(messages, conversationName) {
    let humanCount = 0;
    let claudeCount = 0;
    let skippedCount = 0;
    let warningCount = 0;
    ...
    for (const msg of messages) {
      ...
      for (const block of contentBlocks) {
        ...
        if (block.type === 'text') {
          ...
          // Use export-pdf-serif.js for full output.
          omittedCount++;
    ...
    const omittedNote = omittedCount > 0
      ? `<p class="omitted-note">ℹ️ ${omittedCount} technical block` +
        ...
        `results, extended thinking). Use export-pdf-serif.js for ` +
        `full output.</p>`
      : '';
...
<body>
  ...
  try {
    ...
    const {
      ...
      warningCount,
      omittedCount
    } = buildPdfCleanSerif(messages, conversationName);
    ...
    console.log(
      `[true-ai-export] PDF clean serif v1.2.0 — ` +
